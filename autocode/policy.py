...
def build_prompt(row: Row, recovery: bool = False) -> str:
    ...
    return (
        ...
        f"{prior_block}"
        "Rules:\n"
        "- COMPLETION (highest priority, fixes goal_incomplete): Once Goal met by real edits + verification evidence (tests/lint/pass/pushed) with clean tail (no blockers/next/todo), IMMEDIATELY emit ONLY FLEET_DONE as final line. Do NOT use FLEET_PLAN or FLEET_MILESTONE on turns where objective is satisfied - lenient assess now accepts verified clean work. Be decisive, do not over-require 'nothing left'.\n"
        ...
        "- Always end your response with exactly one structured marker line (FLEET_PLAN or FLEET_MILESTONE or FLEET_DONE). Put it as the very last line. Prefer FLEET_DONE for completed goals per rule above; FLEET_PLAN only for initial decomposition; FLEET_MILESTONE_COMPLETE only for true partials with remaining work.\n"
        ...
        "- Decomposition optional (fixes goal_incomplete): emit FLEET_PLAN (3-5 subtasks) ONLY on first turn IF goal requires multiple turns; for any turn where you complete the objective per COMPLETION rule (edits+tests+verification, clean tail), emit ONLY FLEET_DONE as last line -- even if no prior decomp. Never force PLAN/MILESTONE when objective satisfied. Infer next subtask from context if needed.\n"
